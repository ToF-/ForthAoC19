...
: CROSS-INTERSECT? ( q,p0,pN,p,q0,qN -- f )
    2>R -ROT       ( q,p,p0,pN )
    1+ WITHIN      ( q,p0<=p<=pN )
    SWAP 2R>       ( p0<=p<=pN,q,q0,qN )
    1+ WITHIN      ( p0<=p<=pN,q0<=q<=qN )
    AND ;          ( f )

: HORIZONTAL? ( d -- f )
    DUP RIGHT-DIR = SWAP LEFT-DIR = OR ;

: VERTICAL? ( d -- f )
    DUP UP-DIR = SWAP DOWN-DIR = OR ;

: CROSS-DIRECTION? ( d,e -- f )
    HORIZONTAL? SWAP VERTICAL? XOR 0= ;

: ADD-CROSS-INTERSECTION ( d,q,p0,pN,e,p,q0,qN )
    2ROT 2SWAP           ( d,q,e,p,p0,pN,q0,qN )
    2>R 2>R              ( d,q,e,p )
    ROT 2SWAP            ( p,q,d,e )
    2DUP CROSS-DIRECTION? IF ( p,q,d,e )
        DROP DUP UP-DIR =    ( p,q,d,d=up )
        SWAP DOWN-DIR = OR   ( p,q,d=vert )
        IF SWAP THEN         ( q,p )
        2R> 2>R              ( q,p,p0,pN,q0,qN )
        2ROT 2SWAP           ( p0,pN,q,p,q0,qN )
...
LEFT-DIR LEFT-DIR CROSS-DIRECTION? .
RIGHT-DIR LEFT-DIR CROSS-DIRECTION? .
LEFT-DIR RIGHT-DIR CROSS-DIRECTION? .
RIGHT-DIR RIGHT-DIR CROSS-DIRECTION? .
DOWN-DIR DOWN-DIR CROSS-DIRECTION? .
UP-DIR DOWN-DIR CROSS-DIRECTION? .
DOWN-DIR UP-DIR CROSS-DIRECTION? .
UP-DIR UP-DIR CROSS-DIRECTION? .
DOWN-DIR LEFT-DIR CROSS-DIRECTION? .
LEFT-DIR DOWN-DIR CROSS-DIRECTION? .
DOWN-DIR RIGHT-DIR CROSS-DIRECTION? .
RIGHT-DIR DOWN-DIR CROSS-DIRECTION? .
UP-DIR LEFT-DIR CROSS-DIRECTION? .
LEFT-DIR UP-DIR CROSS-DIRECTION? .
UP-DIR RIGHT-DIR CROSS-DIRECTION? .
RIGHT-DIR UP-DIR CROSS-DIRECTION? .
