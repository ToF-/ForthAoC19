...
CREATE INTERSECTION-LINES 100 2* CELLS ALLOT

: INTERSECTION-LINES! ( j,i -- )
    2DUP SWAP ." BETWEEN LINE# " . ." AND # " .
    INTERSECTION-LINES MAX-INTERSECTION @ 2* CELLS + 2! ;
    ...
    0 DO                       ( addr1,size1,addr2 )
        OVER 0 DO                ( addr1,size1,addr2 )
            ...
            INTERSECTION? IF     ( addr2,addr1,size1,row,col )
                J I INTERSECTION-LINES!
                2DUP SWAP ." INTERSECTION AT " . . CR
                COORDS>CELL
                INTERSECTIONS
